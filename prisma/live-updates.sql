-- Transactional invalidations consumed by the standalone live gateway.
-- PostgreSQL delivers NOTIFY only after commit. This feed is an invalidation
-- mechanism, not a durable log or an authorization boundary.
--
-- This script is idempotent and intentionally lives outside prisma/migrations:
-- Prisma cannot model triggers, and the gateway installs it after `prisma db
-- push` has created the ordinary schema.
...
BEGIN
    ...
    IF TG_TABLE_NAME IN ('ManagementUser', 'ManagementSession', 'ManagementAccount') THEN
        change_kind := CASE WHEN TG_TABLE_NAME = 'ManagementUser' THEN 'user' ELSE 'session' END;
        FOR target_user_id IN
            SELECT DISTINCT value
            FROM unnest(CASE WHEN TG_TABLE_NAME = 'ManagementUser'
...
BEGIN
    FOREACH target_table IN ARRAY ARRAY[
        'Event', 'Quota', 'Question', 'Signup', 'Answer',
        'ManagementUser', 'ManagementSession', 'ManagementAccount'
    ] LOOP

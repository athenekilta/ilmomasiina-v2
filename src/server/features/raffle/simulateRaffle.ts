...
interface Position {
  ...
  radius: number;
  stopped: boolean;
  stoppedAt?: number; // Track when the ball stopped
}
...
export function simulateRaffle(participants: Participant[], seed: string, canvasWidth: number = 1200) {
  const frames: Array<{
    time: number;
  ...
  function checkPegCollision(pos: Position, peg: Peg) {
  ...
  const finalPositions = positions
    .map((pos, index) => {
      ...
      return {
        id: participant.id,
        name: participant.name,
        stoppedAt: pos.stoppedAt ?? Infinity
      };
    })
